var search_image_item = {};
search_image_item.init = function () {
    if ($("#image-list").length === 0) {
        return false;
    }
    var FULL_WIDTH = $('#image-list').width()
        , AVERAGE_HEIGHT = 100;
    if (FULL_WIDTH > 600) {
        AVERAGE_HEIGHT = 160
    }
    $('.search-image-item-img').addClass('no-resized').css('height', AVERAGE_HEIGHT).css('display', 'block');
    search_image_item.resize();
};
search_image_item.resize = function () {
    if ($("#image-list").length === 0) {
        return false;
    }
    var FULL_WIDTH = $('#image-list').width()
    ...
    var $IMG_LIST = $('.search-image-item-img.no-resized')
        , START_INDEX = 0
        , END_INDEX = 0
        , TOTAL_WIDTH = 0
        , $THIS
        ...
        , LINE_NUMBER = 1
        , W = $("body").attr("data-w");
    $('.search-image-item-img.no-resized').css('height', AVERAGE_HEIGHT);
    for (var x = 0; x < $IMG_LIST.length; x++) {
        ...
        $THIS.attr('id', 'search-image-item-img-' + x);
    ...
    if ($('.search-image-item-img').length > 0) {
        if ($('.search-image-item-img').length !== $('.search-image-item-img.no-resized').length) {
            if (W === 'img') {
                if ( $('#image-more').css('display') === 'none' ) {
                    $('.search-image-item-img').css('display', 'block');
                } else {
                    $('.search-image-item-img.no-resized').css('display', 'none');
                }
            } else {
                $('.search-image-item-img.no-resized').css('display', 'none');
            }
