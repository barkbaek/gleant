const model = require('../models/single_apply_now');
...
module.exports = function (lang, is_mobile, is_loginned, user, doc, template) {
    template.render('single_apply_now', {
        lang: function () {
            if (lang === undefined) {
                lang = "en";
            }
        ...
        title: function () {
            return doc.title + " - " + i18n[lang].apply_now + " - " + i18n[lang].employment_obj.title;
        },
        ...
        url: function () {
            return config["url"] + '/apply-now/' + doc._id;
        },
        alternate_list: function () {
            var list = [];
            var url = config["url"] + '/apply-now/' + doc._id;
        ...
        realtime_apply_now: function () {
            if (
                lang === "en" ||
                lang === "ko"
            ) {
                str = i18n[lang].realtime  + " " + i18n[lang].apply_now;
            } else {
                str = i18n[lang].realtime + i18n[lang].apply_now;
            }
            return str;
        ...
        apply_now: function () {
            ...
            if (user === null) {
                return article_templates.get_single_perfect_employment(lang, doc, is_loginned, null);
            } else {
                return article_templates.get_single_perfect_employment(lang, doc, is_loginned, user["blog_id"]);
            }
        }
    });
};